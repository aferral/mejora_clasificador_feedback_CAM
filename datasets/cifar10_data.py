...
class Cifar10_Dataset(Dataset):
    ...
    def __init__(self,epochs,batch_size):
        ...
        self.valid_dataset = dataset_train.map(preprocess).shuffle(500).repeat(epochs).batch(batch_size).cache().prefetch(2000)
        self.dataset_test = dataset_train.map(preprocess).shuffle(500).repeat(epochs).batch(batch_size).cache().prefetch(2000)
